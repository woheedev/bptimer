...
import type { MobWithChannels } from '$types/runtime';
...
async function getMobsByType(
	...
	try {
		...
		const statuses_by_mob = new Map<string, Map<number, ChannelReportData>>();
		...
		const mobs_with_channels = records.map((mob) => {
			...
			const channel_reports: ChannelReportData[] = channel_statuses
